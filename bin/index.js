...
const argv = require('yargs')
  ...
  .describe('client-secret', 'For authentication using an Adobe I/O integration. Your client secret. You can find this on the overview screen for the integration you have created within the Adobe I/O console (https://console.adobe.io). Optionally, rather than passing the client secret as a command line argument, it can instead be provided by setting an environment variable named REACTOR_UPLOADER_CLIENT_SECRET.')
...
(async () => {
  try {
    ...
    const extensionPackageName = await getExtensionPackageName(zipPath);
    let extensionPackageId = await getExtensionPackageId(envSpecificConfig, accessToken, extensionPackageName);
